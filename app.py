...
# ---------------------------------------------------------
# We pull text from the official NAWASA site so the assistant is
# grounded in real, current content instead of only the hardcoded
# facts below. Cached for an hour so we're not re-fetching on every
# chat message.
...
def fetch_website_context(urls):
    ...
    for url in urls:
        ...
        try:
            ...
            resp = requests.get(
                url,
                timeout=8,
                headers={"User-Agent": "Mozilla/5.0 (NAWASA Assist bot)"},
            )
            ...
            # Keep it bounded so the prompt doesn't get huge.
            chunks.append(f"[From {url}]\n{text[:6000]}")

        except Exception as e:

            chunks.append(f"[Could not fetch {url}: {e}]")

    return "\n\n".join(chunks)


WEBSITE_CONTEXT = fetch_website_context(tuple(WEBSITE_URLS))
...
CONTENT FROM THE OFFICIAL NAWASA WEBSITE (nawasa.gd) - prefer this
over the facts above if they ever conflict, since this is pulled
live from the site:

{website_context}
""".replace("{website_context}", WEBSITE_CONTEXT or "(Website content unavailable right now.)")
...
def render_chat_section():
    ...
    if not key_present:
        ...
        st.caption("🟢 Connected to Gemini, grounded with nawasa.gd + live search.")
